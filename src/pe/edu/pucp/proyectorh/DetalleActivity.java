	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState == null) {
			...
			if (Modulo.MODULO_ACTUAL == Constante.LINEA_DE_CARRERA) {
				if ("1".equals(getIntent().getStringExtra(
						DetalleFragment.ARG_ITEM_ID))) { // Comparar capacidades
				...
				if ("2".equals(getIntent().getStringExtra(
						DetalleFragment.ARG_ITEM_ID))) { // Candidatos por
															// puesto
			...
			if (Modulo.MODULO_ACTUAL == Constante.REPORTES) {
				if ("3".equals(getIntent().getStringExtra(
						DetalleFragment.ARG_ITEM_ID))) {
					...
					&& ("4".equals(getIntent().getStringExtra(
							DetalleFragment.ARG_ITEM_ID)))) {
			...
			if ("4".equals(getIntent().getStringExtra(
					DetalleFragment.ARG_ITEM_ID))) {
				ReporteObjetivosBSCPrincipal fragment = new ReporteObjetivosBSCPrincipal();
				getSupportFragmentManager().beginTransaction()
						.replace(R.id.opcion_detail_container, fragment)
